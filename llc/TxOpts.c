...
/// TxOpts undefined setting (used for Service field)
#define TXOPTS_UNDEFINED               (0xFF)
/// Multicast bit of the first byte of a 6 byte 802.11 MAC address
#define MULTICAST_ADDRESS_BIT_TOP_BYTE (0x01)
                     ...
                     char *IndentStr)
{
  ...
  printf("BSSID Address: %02x:%02x:%02x:%02x:%02x:%02x\n",
         pTxCHOpts->BSSAddr[0], pTxCHOpts->BSSAddr[1],
         pTxCHOpts->BSSAddr[2], pTxCHOpts->BSSAddr[3],
         pTxCHOpts->BSSAddr[4], pTxCHOpts->BSSAddr[5]);
...
void TxOpts_PrintUsage ()
{
  ...
  static tUsageOption UsageOptionList[] =
  {
    ...
    { 'b', "BSSAddr",        "BSSID MAC Address (Address3)", "aa:bb:cc:dd:ee:ff" },
    { 'u', "UDPforwardPort", "UDP forwarding port", "12345" },
    { 'k', "TxEventData",    "Output Tx Event Data", ""},
    { 0, 0, 0, 0 }
  };
...
void TxOpts_Print (tTxOpts * pTxOpts)
{
  ...
  printf("Output TxEventData %d\n", pTxOpts->TxEventData);
  if (pTxOpts->TxEventData)
  {
    fprintf(stdout, "TxStatus MACSeq#           TxTime  AckResp CTSResp MACDwell "
            "#ShortRetry #LongRetry           DestAddr\n");
  }
}
...
int TxOpts_New (int argc, char **argv, tTxOpts *pTxOpts)
{
  ...
  unsigned char bss_mac[6] = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff };
  ...
  static struct option long_options[] =
  {
    ...
    { "BSSAddr",        required_argument, 0, 'b' },
    { "UDPforwardPort", required_argument, 0, 'u' },
    { "TxEventData",    no_argument,       0, 'k' },
    { "wsmp_n_extension_ind",   no_argument,       0, 'A' },
    { "psid",   required_argument,       0, 'B' },
    { "wsa_ind",   no_argument,       0, 'C' },
    { "wsa_extension_ind",   no_argument,       0, 'D' }, // WSA extension include(2d location, service index, channelindex, WRA)
    { "content",      required_argument, 0, 'E' },
    { "channelnumber_ind",           no_argument,       0, 'F' },
    { "datarate_ind",          no_argument,       0, 'G' },
    { "txpower_ind",    no_argument,       0, 'H' },
    { 0, 0, 0, 0 }
  };
#endif
  // Default settings
  ...
  // disable output of Tx Event Data
  pTxOpts->TxEventData = false; // set true if option present

  //inserted options by ww
  pTxOpts->wsa_extension_ind=false;
  pTxOpts->wsmp_n_extension_ind=false;
  pTxOpts->psid=123;
  pTxOpts->wsa_ind=false;
  pTxOpts->wsa_extension_ind=false;
  pTxOpts->psid=0x00;
  pTxOpts->channelnumber_ind=false;
  pTxOpts->datarate_ind=false;
  pTxOpts->txpower_ind=false;


  // CCH Tx Descriptor Lists
  pTxOpts->TxCHOpts.ChannelNumber = TEST_DEFAULT_CHANNELNUMBER;
  pTxOpts->TxCHOpts.PacketLength[0].Start = 10;
  pTxOpts->TxCHOpts.PacketLength[0].Step = 1; // no range
  pTxOpts->TxCHOpts.PacketLength[0].Stop = 10;
  ...
  // default broadcast bssid
  memcpy(pTxOpts->TxCHOpts.BSSAddr,  bss_mac, ETH_ALEN);
  pTxOpts->TxCHOpts.UDPforwardSocket = -1;

#if defined(__QNX__)
  strcpy(short_options, "s:n:r:f:hyog:i:tz:c:l:m:w:p:x:q:v:a:e:d:b:A:B:C:D:E:F:G:H:");
#else
...
#ifndef __QNX__
  // Reset the option parsing starting point
  optind = 0;
#endif
  ...
  while (1)
  {
    ...
    switch (c)
    {
      ...
      case 'k': // output Tx Event Data? Yes if present
        pTxOpts->TxEventData = true;
        break;
...
#undef TRY_PARSE_MCS
    ...
    case 'b': // BSSID
        if (!(GetMacFromString(pTxCHOpts->BSSAddr, optarg)))
        {
          ErrCode = TXOPTS_ERR_INVALIDOPTIONARG;
          goto Error;
        }
      ...
      case 'A': //wsmp_n_extension (channel#, datarate, Txpower)
        pTxOpts->wsmp_n_extension_ind = true;
        break;

      case 'B': //
        pTxOpts->psid = strtoul(optarg, NULL, 0);
        break;

      case 'C':
        pTxOpts->wsa_ind = true;
        break;

      case 'D': //wsa_extension_options (service index, channel index, WRA, 2d location)
        pTxOpts->wsa_extension_ind = true;
        break;

      case 'E':
    	//strcat((optarg),);
        strcpy((pTxOpts->content),optarg);
        break;
      case 'F': //wsa_extension_options (service index, channel index, WRA, 2d location)
        pTxOpts->channelnumber_ind = true;
        break;
      case 'G': //wsa_extension_options (service index, channel index, WRA, 2d location)
        pTxOpts->datarate_ind = true;
        break;
      case 'H': //wsa_extension_options (service index, channel index, WRA, 2d location)
        pTxOpts->txpower_ind= true;
        break;
...
Exit:
  // Do final check on Ack Policy
  if (pTxOpts->TxCHOpts.Service == TXOPTS_UNDEFINED)
  {
    // When Ack Policy is undefined, assign ack policy from multicast bit
    if ((pTxOpts->TxCHOpts.DestAddr[0] & MULTICAST_ADDRESS_BIT_TOP_BYTE) == 0)
      pTxOpts->TxCHOpts.Service = MK2_QOS_ACK;
    else
