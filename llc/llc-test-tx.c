...
typedef struct LLCTx
{
  ...
  bool Exit;
  FILE *fp;
  /// Timestamp of the previous transmission
  struct timeval TxTime;
  /// Ethernet header (for preloading invariants)
  struct ethhdr EthHdr;
  bool TxContinue;
  /// pthread for calling MKx_Recv
  pthread_t MKxRecvThr;
  /// Control for thread
  bool MKxRecvStop;
} tLLCTx;
...
struct PluginCmd TxCmd =
{
  ...
  .pHelp = (
    ...
    "        aa:bb:cc:dd:ee:ff\n"
    "-b, --BSSAddr\n"
    "        BSSID MAC Address (Address3)\n"
    "        aa:bb:cc:dd:ee:ff\n"
    "-u, --UDPforwardPort\n"
    "        UDP forwarding port\n"
    "        12345\n"
    "-k, --TxEventData\n"
    "        Output the Tx Event Data to stdout\n"),
  .Func = LLC_TxCmd,
};
...
/**
 * @brief Thread which continually calls MKx_Recv()
 */
void *MKxRecvFn(void *pDevHandle)
{
  struct LLCTx *pDevLocal = (struct LLCTx *)pDevHandle;
  if (pDevLocal)
  {
    while (!pDevLocal->MKxRecvStop)
    {
      MKx_Recv(pDevLocal->pMKx);
    }
  }
                                      ...
                                      tTxCHOpts *pTxCHOpts)
{
  ...
  tIEEE80211QoSHeader *pMAC = NULL;
  tSNAPHeader *pSNAP = NULL;
  ...
  //--------------------------------------------------------------------------
  // WAVE-RAW frame: | TxDesc | 802.11 QoS header | SNAP/EPD (Ethertype) | Protocol & Payload |
  pMAC = (tIEEE80211QoSHeader *)pPacket->TxFrame;
  if (pTxCHOpts->Priority == MK2_PRIO_NON_QOS)
  {
    pSNAP = (struct SNAPHeader *) (pPacket->TxFrame + sizeof(tIEEE80211Header));
    pPayload = (unsigned char *) (pPacket->TxFrame + sizeof(tIEEE80211Header) +
                                  sizeof(tSNAPHeader));
  }
  else
  {
    pSNAP = (struct SNAPHeader *) (pPacket->TxFrame + sizeof(tIEEE80211QoSHeader));
    pPayload = (unsigned char *) (pPacket->TxFrame + sizeof(tIEEE80211QoSHeader) +
                                  sizeof(tSNAPHeader));
  }
  ...
  QoSHdr_fprintf(fp, pMAC);
  fprintf(fp, "0x%04x ", ntohs(pSNAP->EtherType));
...
static int Tx_Send (tTxOpts * pTxOpts, int Pause_us)
{
  ...
  int m, r, a; // loop vars
  int ThisFrameLen;
  struct IEEE80211QoSHeader *pMAC; //have to use for plugfest!
  struct IEEE80211Header *pMACNonQoS;
  struct SNAPHeader *pSNAP; //LLC Header
  ...
  if (pTxCHOpts->Priority == MK2_PRIO_NON_QOS)
  {
	printf("Non QoS header making \n");
    pSNAP = (struct SNAPHeader *) (pPacket->TxFrame + sizeof(*pMACNonQoS));
    pPayload = (unsigned char *) (pPacket->TxFrame + sizeof(*pMACNonQoS) +
		                                  sizeof(*pSNAP));
  }
  else
  {
    pSNAP = (struct SNAPHeader *) (pPacket->TxFrame + sizeof(*pMAC));
    pPayload = (unsigned char *) (pPacket->TxFrame + sizeof(*pMAC) +
		                                  sizeof(*pSNAP)-8);
  }
  ...
  if (pTxCHOpts->Priority == MK2_PRIO_NON_QOS)
    pMAC->FrameControl.Fields.SubType = MAC_FRAME_SUB_TYPE_DATA;
  else
  ...
  memcpy(pMAC->Address3, pTxCHOpts->BSSAddr, ETH_ALEN);
  //memset(pMAC->Address3, 0xFF, ETH_ALEN);
  pMAC->SeqControl.SeqCtrl = 0x0000; // Set by the UpperMAC
  if (pTxCHOpts->Priority != MK2_PRIO_NON_QOS)
  {
  ...
  // Setup the EtherType header
  pSNAP->EtherType = htobe16(pTxCHOpts->EtherType);
  ...
  for (m = 0; m < pTxCHOpts->NMCS; m++)
  {
    ...
    for (TxPower = pTxPowerRangeSpec->Start; TxPower <= pTxPowerRangeSpec->Stop;
         TxPower += pTxPowerRangeSpec->Step)
    {
      ...
      for (r = 0; r < pTxCHOpts->NPacketLengths; r++)
      {
        ...
        //{
          ...
          for (a = 0; a < pTxCHOpts->NTxAnt; a++)
          {
            ...
            //printf("RadioID:%d,channel number:%d,txantenna:%d,MCS:%d,Txpower:%ld",pPacket->RadioID,pTxCHOpts->ChannelNumber,pTxCHOpts->pTxAntenna[a],pTxCHOpts->pMCS[m],TxPower);
            ...
            if (pTxCHOpts->UDPforwardSocket >= 0)
            {
                ...
                PayloadLen = read(pTxCHOpts->UDPforwardSocket, pPacket->TxFrame,
                                  Size);
                    overhead = 0;
                }
            ...
            else
            {
              ...
              Tx_fprintf(pDev->fp, pPacket, ThisFrameLen,
                         pTxOpts->DumpPayload, DumpHeadings, pTxCHOpts);
            ...
            // Extra delay in order to achieve the Pause_us period in between
            // transmissions (attempt to set tx rate)
            usleep(Pause_us);
...
static int Tx_SendAtRate(tTxOpts * pTxOpts)
{
  ...
  while ( (pDev->SeqNum < pTxOpts->NumberOfPackets) && (pDev->TxContinue) )
  {
    ...
    // if many then update rate control, unless per-event data output is enabled
    if ((ThisNPackets >= NPacketsPerCheck) && (pDev->TxOpts.TxEventData == 0))
    {
...
int LLC_OpenUDP(uint16_t Port)
{
  ...
  {
    d_fnend(D_ERR, NULL,
            "setsockopt(%d, IPPROTO_IPV6, IPV6_V6ONLY, 0) failed: %d %s\n",
  ...
  {
    d_error(D_ERR, NULL,
            "setsockopt(%d, SOL_SOCKET, SO_REUSEADDR, 1) failed: %d %s\n",
            Fd, errno, strerror(errno));
    close(Fd);
    return -1;
  }
                      ...
                      void *pPriv)
{
  ...
  if (pDev->TxOpts.TxEventData)
  {
    //fprintf(stdout, "%8d %7d %12" PRIu64 " %7u %7u %8u %11u %10u\n",
    fprintf(stdout, "%8d %7d %16" PRIu64 " %7u %7u %8u %11u %10u ",
            pTxEvent->TxEventData.TxStatus,
            pTxEvent->TxEventData.MACSequenceNumber,
            pTxEvent->TxEventData.TxTime,
            pTxEvent->TxEventData.AckResponseDelay_ps, 
            pTxEvent->TxEventData.CTSResponseDelay_ps, 
            pTxEvent->TxEventData.MACDwellTime,
            pTxEvent->TxEventData.NumShortRetries,
            pTxEvent->TxEventData.NumLongRetries);
    MACAddr_fprintf(stdout, pTxEvent->TxEventData.DestAddress);
    fprintf(stdout, "\n");
  }
...
int LLC_TxInit (struct LLCTx *pDev)
{
  ...
  pTxOpts->TxCHOpts.UDPforwardSocket =
    LLC_OpenUDP(abs(pTxOpts->UDPforwardPort));
  ...
  // Initialise the last transmit time with the current time
  gettimeofday(&pDev->TxTime, NULL);

  pDev->MKxRecvStop = 0;
  if (pthread_create(&pDev->MKxRecvThr, NULL, MKxRecvFn, pDev))
  {
    fprintf(stderr, "Fail: error creating thread\n");
    Res = TX_ERR_NULLFILEPOINTER;
    goto Error;
  }
...
int LLC_TxExit (struct LLCTx *pDev)
{
  ...
  // Stop the MKx_Recv thread
  pDev->MKxRecvStop = 1;
  if (pthread_join(pDev->MKxRecvThr, NULL))
  {
    fprintf(stderr, "Error joining thread\n");
    return TX_ERR_NULLFILEPOINTER;
  }
